...
export function connectWebSocket(
  ...
  stompClient.connect({}, () => {
    ...
    stompClient.subscribe(`/v1/sub/chat/rooms/${chatRoomId}/list`, (message) => {
      const initialMessages = JSON.parse(message.body);

      if (onMessageReceived) {
        onMessageReceived(initialMessages);
      }
    });
    ...
    stompClient.subscribe(`/v1/sub/chat/rooms/${chatRoomId}`, (message) => {
      ...
      if (onMessageReceived) {
        onMessageReceived(parsedMessages);
      }
    });
  });
...
// WebSocket 연결 해제 함수
export function disconnectWebSocket(stompClient: CompatClient): void {
  if (stompClient) {
    // DISCONNECT
